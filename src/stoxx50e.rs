...
impl Debug for Stoxx50e {
    fn fmt(&self, _: &mut Formatter) -> Result<(), Error> {
        println!("Euro Stoxx 50: {}", self.value);
...
impl Stoxx50e {
    pub fn new() -> Stoxx50e {
        let html = Stoxx50e::download_source();
        if html.is_err() {
            return Stoxx50e{
                indizes: Vec::new(),
                value: -1.234
        ...
        Stoxx50e {
            indizes: Stoxx50e::scrape_indizes(&source),
            value: Stoxx50e::scrape_value(&source)
        }
    }

    fn download_source() -> Result<Document, ()> {
        reqwest::get("http://www.boerse-online.de/index/liste/Euro_Stoxx_50")
            .map_err(|_| ())
    ...
    fn scrape_value(src: &Document) -> f32 {
        ...
        let val: String = src.find(Attr("data-item", "Y0306000000DJSTOXXE50"))
            .skip(1)
            .next()
            .map(|x| x.html())
            .unwrap_or(String::new());
